...
class GeneticOptimizer(OptimizationMethod):
    """
    Genetic algorithm optimization for spin systems.
    """
    ...
    def optimize(
        ...
        **kwargs
        ...
        population = self._initialize_population(initial_config, population_size)
        orientations = self.optimizer.spin_system.generate_spin_orientations()
        ...
        if verbose:
            pbar = tqdm(total=max_iterations, desc="GA Optimization")
        
        for generation in range(max_iterations):
            # Evaluate population
            energies = []
            for individual in population:
                energy = self.optimizer.objective_function(individual)
                energies.append(energy)
            ...
            # Check convergence
            if len(energies_history) > 10:
                recent_improvement = energies_history[-10] - energies_history[-1]
            ...
            while len(new_population) < population_size:
                # Selection
                parent1 = self._tournament_selection(population, energies)
                parent2 = self._tournament_selection(population, energies)
                ...
                if np.random.random() < mutation_rate:
                    child1 = self._mutate(child1, orientations)
                if np.random.random() < mutation_rate:
                    child2 = self._mutate(child2, orientations)
            ...
            # Callback
            if callback is not None:
                callback(self.optimizer, generation, best_config)
            
            if verbose:
                pbar.set_postfix({
                    'Best Energy': f'{best_energy:.6f}',
                    'Avg Energy': f'{np.mean(energies):.6f}'
                })
                pbar.update(1)
        ...
        return {
            'converged': generation < max_iterations - 1,
            'final_configuration': best_config,
            'final_energy': best_energy,
            'iterations': generation + 1,
            'energy_history': np.array(energies_history)
        }
    
    def _initialize_population(self, initial_config: np.ndarray, population_size: int) -> list:
        """Initialize random population."""
        population = [initial_config.copy()]
        
        for _ in range(population_size - 1):
            config = self.optimizer.spin_system.random_configuration()
            population.append(config)
        
        return population
    
    def _tournament_selection(self, population: list, energies: np.ndarray, tournament_size: int = 3) -> np.ndarray:
        """Tournament selection."""
        tournament_indices = np.random.choice(len(population), tournament_size, replace=False)
        tournament_energies = energies[tournament_indices]
        winner_idx = tournament_indices[np.argmin(tournament_energies)]
    ...
    def _mutate(self, individual: np.ndarray, orientations: np.ndarray) -> np.ndarray:
        """Mutate individual by flipping random spins."""
        mutated = individual.copy()
        n_mutations = max(1, int(len(individual) * 0.05))  # Mutate 5% of spins
        
        for _ in range(n_mutations):
            spin_idx = np.random.randint(len(individual))
            orientation_idx = np.random.randint(len(orientations))
